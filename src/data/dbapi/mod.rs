...
pub fn insert_candles(table_name: &str, candles: &[Candle]) -> Result<()> {
    ...
    let (get_table, get_row) = match supported_table {
        SupportedTable::UsdtBtc(_table) => {
            (|| usdt_btc::table, |t: NaiveDateTime, o:f64, h:f64, l:f64, c:f64, v:f64| (usdt_btc::timestamp.eq(t), usdt_btc::open.eq(o), usdt_btc::high.eq(h), usdt_btc::low.eq(l), usdt_btc::close.eq(c), usdt_btc::volume.eq(v)) )
        },
    };
    ...
    for c in candles {
        payload.push(get_row(c.timestamp, c.open, c.high, c.low, c.close, c.volume));
        row_count += 1;
...
pub fn get_all_candles(table_name: &str) -> Result<Vec<Candle>> {
    ...
    let results = match supported_table {
        SupportedTable::UsdtBtc(table) => {
            table.load::<Candle>(&conn)
        }
    }?;
